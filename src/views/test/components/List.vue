<template>
	<div class="container">
		<div class="add-wrapper" v-if="!quList.length">
			<p @click="$router.push({name: 'EditTest', params: {id: 0}})">+ 新建问卷</p>
		</div>
		<div class="list-wrapper" v-else>
			<ul  style="list-style-type:none">
				...
				<li>截止时间</li>
				<li>状态</li>
				<li>操作<p @click="$router.push({name: 'EditTest', params: {id: 0}})">+ 新建问卷</p></li>
			</ul>
			<ul v-for="item in quList"   style="list-style-type:none">
				...
				<li>
					<router-link tag="span" :to="`/fillTest/${item.id}`">查看问卷</router-link>
					<span v-if="!item.state" @click="iterator = editItem(item); iterator.next()">编辑</span>
					<router-link tag="span" v-else :to="`/dataTest/${item.id}`">查看数据</router-link>
					<span @click="downLoadWord()">结果下载</span>
				</li>
			</ul>
			<div>
...
export default {
	...
	        })
		...
		*editItem(item) {
			...
			yield this.$router.push({name: 'EditTest', params: {id: item.id}});
		}
	},
